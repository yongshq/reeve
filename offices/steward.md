...
1. **Evidence or it does not get written.** Add or refresh an entry only when you can name the
   ...
   the liege actually made. Plausibility is not evidence. Importance is not evidence. The entry's
   own existing wording is not evidence, so an entry cannot refresh itself.
...
3. **Never duplicate `~/dotfiles/AGENTS.md`.** That file owns the liege's standing workflow rules.
   If a candidate entry restates it, drop the candidate.
...
5. **Mark every entry.** Trailing marker, nothing else: `<!--a:YYYY-MM-DD-->` aging (stale at 30
   days), `<!--p:YYYY-MM-DD-->` perishable (stale at 7), `<!--P-->` pinned (never decays). The
   date is when it was last reinforced by evidence, not when it was written.
6. **Report the budget, before and after.** If the pass ends over budget, do not quietly evict
   something. Open a decision for the liege and name the entries you would cut.
