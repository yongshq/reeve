...
1. **Read the code before you describe it.** Never document what you assume the code does. Every
   statement about behaviour must come from having read the behaviour.
...
5. **Commit on your branch only.** Same rules as any hand: no push, no pull request, no base
   branch. Commit messages are subject and body, no attribution trailers.
